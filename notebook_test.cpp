...
(begin
	...
	int tester = 0;
	while (viewwid->items().size() == 0)
	{
		tester++;
		QTest::mouseClick(inwid, Qt::LeftButton, Qt::NoModifier, QPoint(0, 0), 100);
		qDebug() << "\n" << tester <<"00";
	}
	//QTest::mouseClick(inwid, Qt::LeftButton, Qt::NoModifier, QPoint(0, 0), 100);
	QCOMPARE(viewwid->items().size(), 67 + 12);
	QTest::mouseClick(inwid, Qt::LeftButton, Qt::NoModifier, QPoint(0, 0), 10000);
	/*
